...
<body>
	...
	<table id="todoList" border="1">
		...
		<tbody>
			<c:forEach items="${todoList}" var="todo" varStatus="vs"> <%-- todoList 대상으로 하나씩 순회 --%>
				<tr>
					...
					<td>
						<%-- 제목 클릭 시
							...
							서버에서 상세내용 조회 시 todoNo를 이용하게 끔 함
						 --%>
						<a href="/todo/detail?todoNo=${todo.todoNo}">${todo.todoTitle}</a> <%-- 제목을 클릭했을 때 상세페이지로 이동 --%>
					</td>				 <%-- ? 쿼리스트링 --%> <%-- 파라미터 --%>
